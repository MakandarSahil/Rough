...
export const TokenService = {
  ...
  async getTokens(): Promise<AuthTokens> {
    try {
      ...
      const accessToken = (accessResult && typeof accessResult !== 'boolean') ? accessResult.password : null;
      const refreshToken = (refreshResult && typeof refreshResult !== 'boolean') ? refreshResult.password : null;
  ...
  async clearTokens(): Promise<void> {
    try {
      ...
      console.log(`Access Token: ${access_token ? access_token.substring(0, 10) + '...' : 'null'}`);
      console.log(`Refresh Token: ${refresh_token ? refresh_token.substring(0, 10) + '...' : 'null'}`);
    } catch (error) {
